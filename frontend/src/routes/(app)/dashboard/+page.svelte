<script lang="ts">
	...
	const DEFAULT_SECTIONS: ApplicationSectionStatus[] = [
		{ key: 'profile', label: 'Profile', href: '/dashboard/application/profile', complete: false },
        { key: 'contact', label: 'Contact', href: '/dashboard/application/contact', complete: false },
		{ key: 'family', label: 'Family', href: '/dashboard/application/family', complete: false },
		{ key: 'education', label: 'Education', href: '/dashboard/application/education', complete: false },
		{ key: 'testing', label: 'Testing', href: '/dashboard/application/testing', complete: false },
		{
			key: 'activities',
			label: 'Activities and experiences',
			href: '/dashboard/application/activities',
			complete: false
		},
		{ key: 'writing', label: 'Writing', href: '/dashboard/application/writing', complete: false }
	];
	...
	async function loadDashboard() {
		...
		try {
			...
			loadError = err instanceof Error ? err.message : 'Could not load your dashboard.';
		} finally {
			loading = false;
		}
	}
	...
	const visibleSections = $derived(
		summary.applicationSections.length ? summary.applicationSections : DEFAULT_SECTIONS
	);
	const totalSections = $derived(visibleSections.length);
	const completedSections = $derived(visibleSections.filter((s) => s.complete).length);
	...
	const dashOffset = $derived(circumference - (percentComplete / 100) * circumference);
</script>
...
<div class="dashboard-page">
	...
	<section class="hero">
		<svg class="hero-illustration" width="140" height="90" viewBox="0 0 140 90" aria-hidden="true">
			<circle cx="112" cy="20" r="14" fill="#ffd76a" />
			<path d="M20 78c0-22 18-38 38-38s38 16 38 38" fill="none" stroke="#1a56db" stroke-width="3" />
		...
		{#if applicationExpanded}
			<div class="card-body application-body">
				<div class="circular-progress" style="width:{ringSize}px">
					<svg width={ringSize} height={ringSize} viewBox="0 0 {ringSize} {ringSize}" role="img" aria-label="{percentComplete}% complete">
						<circle cx={ringSize / 2} cy={ringSize / 2} r={radius} fill="none" stroke="#e2e8f0" stroke-width={strokeWidth} />
						<circle
							cx={ringSize / 2}
							cy={ringSize / 2}
							r={radius}
							...
							class="progress-arc"
						/>
						<text x="50%" y="50%" text-anchor="middle" dominant-baseline="central" class="center-text">
							{percentComplete}%
						</text>
					</svg>
					<p class="progress-label">{completedSections}/{totalSections} sections complete</p>
				</div>
							...
							<span class="icon" class:complete={section.complete}>
								{#if section.complete}
									<svg viewBox="0 0 24 24" width="18" height="18" fill="none" aria-hidden="true">
										<path d="M5 12.5l4.5 4.5L19 7" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
									</svg>
								{/if}
							</span>
	...
	<section class="card">
		...
		<div class="card-body">
			<p class="muted">{loading ? '—' : summary.universitiesOnList} universities on my list</p>
			...
			<button class="link-button" onclick={() => (universitiesExpanded = !universitiesExpanded)}>
				{universitiesExpanded ? '▴ Hide universities' : '▾ Show universities'}
			</button>

			{#if universitiesExpanded}
				<a class="link-button" href="/dashboard/universities">Go to My Universities →</a>
			{/if}
		</div>
	</section>
</div>
